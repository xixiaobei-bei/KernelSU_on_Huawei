...
/*
 * New-style supercall handshake (ksud >= ~v1.1):
 * ksud calls sched_setaffinity(pid=0xDEADBEEF, cpusetsize=0xC,
 *                               mask=<arg ptr>)
 * to install the KSU fd.  We intercept it here and reuse the existing
 * ksu_handle_sys_reboot() logic which already handles KSU_INSTALL_MAGIC*.
 *
 * x0 = pid   (magic1)
 * x1 = cpusetsize (magic2)
 * x2 = user mask ptr (arg, used as reply pointer)
 */
static int setaffinity_handler_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	/* Only act on our magic values; let normal calls pass through */
	if ((unsigned int)magic1 != KSU_INSTALL_MAGIC1 &&
	    (unsigned int)magic1 != (unsigned int)CHANGE_MANAGER_UID &&
	    (unsigned int)magic1 != (unsigned int)CHANGE_KSUVER &&
	    (unsigned int)magic1 != (unsigned int)CHANGE_SPOOF_UNAME &&
	    (unsigned int)magic1 != (unsigned int)GET_SULOG_DUMP_V2)
		return 0;

	return ksu_handle_sys_reboot(magic1, magic2, 0, (void __user **)&arg2);
}
...
void __init ksu_supercalls_init(void)
{
  ...
  ksu_supercall_init_handlers();
	...
	rc = register_kprobe(&setaffinity_kp);
	if (rc) {
		pr_warn("setaffinity kprobe failed: %d (new-style handshake unavailable)\n", rc);
	} else {
