...
static int apply_kernelsu_rules_fn(void *ptr)
{
    ...
    ksu_allow(db, "ksu", "adb_data_file", "file", ALL);
    ksu_allow(db, "ksu", "adb_data_file", "dir", ALL);
...
/* switch-case 替换为 if-else：3.18 BSP 中 KSU_SEPOLICY_CMD_* 为 const 变量，
 * 不满足 C 标准对 case label 必须是整型常量表达式的要求。*/
static int sepol_expected_argc(u32 cmd)
{
    if (cmd == KSU_SEPOLICY_CMD_NORMAL_PERM)
        return 4;
    else if (cmd == KSU_SEPOLICY_CMD_XPERM)
        return 5;
    else if (cmd == KSU_SEPOLICY_CMD_TYPE_STATE)
        return 1;
    else if (cmd == KSU_SEPOLICY_CMD_TYPE || cmd == KSU_SEPOLICY_CMD_TYPE_ATTR)
        return 2;
    else if (cmd == KSU_SEPOLICY_CMD_ATTR)
        return 1;
    else if (cmd == KSU_SEPOLICY_CMD_TYPE_TRANSITION)
        return 5;
    else if (cmd == KSU_SEPOLICY_CMD_TYPE_CHANGE)
        return 4;
    else if (cmd == KSU_SEPOLICY_CMD_GENFSCON)
        return 3;
    else
        return -EINVAL;
}
                                  ...
                                  const char **args)
{
    ...
    } else if (header->cmd == KSU_SEPOLICY_CMD_XPERM) {
    ...
    } else if (header->cmd == KSU_SEPOLICY_CMD_TYPE_STATE) {
    ...
    } else if (header->cmd == KSU_SEPOLICY_CMD_TYPE ||
               header->cmd == KSU_SEPOLICY_CMD_TYPE_ATTR) {
        ret = sepol_require_not_all(args[0], "type");
        if (ret < 0)
            return ret;
    ...
    } else if (header->cmd == KSU_SEPOLICY_CMD_ATTR) {
    ...
    } else if (header->cmd == KSU_SEPOLICY_CMD_TYPE_TRANSITION) {
    ...
    } else if (header->cmd == KSU_SEPOLICY_CMD_TYPE_CHANGE) {
        ret = sepol_require_not_all(args[0], "src");
        if (ret < 0)
            return ret;
        ret = sepol_require_not_all(args[1], "tgt");
    ...
    } else if (header->cmd == KSU_SEPOLICY_CMD_GENFSCON) {
