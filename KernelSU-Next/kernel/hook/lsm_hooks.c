...
/* security/security.c must expose this as:
...
void __init ksu_lsm_hook_init(void)
{
	...
	if (ret) {
		 ...
		 * 本 3.18 BSP 的 kallsyms_lookup_name("security_ops") 不可靠，
		 * 因此直接使用 security/security.c 导出的全局 security_ops 指针。
		 */
		struct security_operations **ops_ptr = &security_ops;

		pr_warn("register_security failed (%d), patching exported security_ops directly\n", ret);

		if (!ops_ptr || !*ops_ptr) {
			pr_err("security_ops is NULL, LSM hooks NOT installed!\n");
			return;
		}
