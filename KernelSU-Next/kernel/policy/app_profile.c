...
void setup_groups(struct root_profile *profile, struct cred *cred)
{
	...
	/* groups_sort 在华为 3.18 BSP 中未导出，运行时通过 kallsyms 查找 */
	static void (*__groups_sort)(struct group_info *) = NULL;
	if (!__groups_sort)
		__groups_sort = (void *)kallsyms_lookup_name("groups_sort");
	if (__groups_sort)
		__groups_sort(group_info);
	else
		pr_warn("groups_sort not found via kallsyms, groups may be unsorted!\n");
