...
static const char KERNEL_SU_RC[] =
	...
	"    exec u:r:" KERNEL_SU_DOMAIN ":s0 -- " KSUD_PATH " post-fs-data\n"
	"\n"

	"on nonencrypted\n"
	"    exec u:r:" KERNEL_SU_DOMAIN ":s0 -- " KSUD_PATH " services\n"
	"\n"
	...
	"    exec u:r:" KERNEL_SU_DOMAIN ":s0 -- " KSUD_PATH " boot-completed\n"
	"\n"
...
static bool is_init_rc(struct file *fp)
{
    ...
    /* d_is_reg() 是 3.19+ 引入的，3.18 用 S_ISREG 替代 */
    if (!fp->f_path.dentry->d_inode ||
        !S_ISREG(fp->f_path.dentry->d_inode->i_mode)) {
        return false;
    }
