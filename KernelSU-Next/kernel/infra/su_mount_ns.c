...
try_setns:
    ...
    /*
     * 3.18 没有 ns_get_path()，改用 proc_ns_fget 思路：
     * 直接从 pid1_task->nsproxy->mnt_ns 拿到 mnt_ns，
     * 再通过 mntns_operations.get() 得到对应的 ns_common（即 proc_inode 路径），
     * 最终用 create_ns_file（内部实现）打开。
     *
     * 更简单可靠的方式：直接 open /proc/1/ns/mnt 路径。
     * 这在内核态用 filp_open 完全可行，不依赖任何版本相关 API。
     */
    put_task_struct(pid1_task);

    struct file *ns_file = filp_open("/proc/1/ns/mnt", O_RDONLY, 0);
    if (IS_ERR(ns_file)) {
        pr_warn("failed open /proc/1/ns/mnt: %ld\n", PTR_ERR(ns_file));
        goto out;
    }
