...
static struct ice40_spi_priv_data *g_spi_drv_data;
static struct mutex g_gpio_power_ctrl_lock;
static bool g_ice40_enable_gpio_powerdown;
static bool g_color_set_gpio185_powerup;

static int ice40_spi_read_reg(struct ice40_spi_priv_data *devdata,
	u8 addr, u8 *val);
static int ice40_spi_write_reg(struct ice40_spi_priv_data *devdata,
	u8 addr, u8 val);
 ...
 * bit0: 0
 */
static struct err_map g_fpga_irq_err_table[] = {
	/* 927011003 927011004 927011005 927011006 927011007 927011008 */
	{
		0x04,
		"Projector over current",
		DSM_FPGA_PROJ_OVER_CURRENT
	},
	{
		0x20,
		"Illuminator over current",
		DSM_FPGA_ILLU_OVER_CURRENT
	},
	{
		0x08,
		"Projector pulse too long",
		DSM_FPGA_PROJ_PLUS_TOO_LONG
	},
	{
		0x40,
		"Illuminator pulse too long",
		DSM_FPGA_ILLU_PLUS_TOO_LONG
	},
	{
		0x10,
		"Projector pulse frequency over limit",
		DSM_FPGA_PROJ_FREQU_OVER_LIMIT
	},
	{
		0x80,
		"Illuminator pulse frequency over limit",
		DSM_FPGA_ILLU_FREQU_OVER_LIMIT
	},
};

static struct err_map g_fpga_common_err_table[] = {
	/* 927011000 927011001 927011002 927011009 927011010 927011011 */
	[LOAD_FW_ERR] = {
		0x01,
		"FPGA load image error",
		DSM_FPGA_LOAD_FW_FAIL
	},
	[SPI_RW_ERR] = {
		0x01,
		"FPGA SPI error",
		DSM_FPGA_SPI_RW_FAIL
	},
	[SELF_TEST_ERR] = {
		0x01,
		"FPGA Self Test error",
		DSM_FPGA_SELT_TEST_FAIL
	},
	[AGE_PROJ_ERR] = {
		0x01,
		"Ageing projector pulse frequency over limit",
		DSM_FPGA_AGEING_PROJ_FREQU_LIMIT
	},
	[AGE_ILLU_ERR] = {
		0x01,
		"Ageing illuminator pulse frequency over limit",
		DSM_FPGA_AGEING_ILLU_FREQU_LIMIT
	},
	[OTHER_FILL_ERR] = {
		0x01,
		"FPGA other failed",
	...
	char *err_name, u32 err_code)
{
	...
	if (!dsm_client_ocuppy(drv_data->client_ice40)) {
		dsm_client_record(drv_data->client_ice40,
			"[ice40] %s.\n", err_name);
		dsm_client_record(drv_data->client_ice40,
			"[ice40] 0x02 = 0x%02x.\n",
...
static void fpga_load_firmware_notify(struct ice40_spi_priv_data *drv_data)
{
	...
	if (drv_data->load_fw_fail_time > 0) {
		ret = snprintf_s(buff, MAX_BUFF_SIZE,
			MAX_BUFF_SIZE - 1, "%s, fail times: %d.",
			g_fpga_common_err_table[LOAD_FW_ERR].err_name,
			drv_data->load_fw_fail_time);
		if (ret < 0) {
			cam_err("snprintf_s failed %s()%d", __func__, __LINE__);
			return;
		}

		fpga_dsm_client_notify(drv_data, buff,
			g_fpga_common_err_table[LOAD_FW_ERR].err_num);
	}

	if (drv_data->self_check_fail_time > 0) {
		ret = memset_s(buff, MAX_BUFF_SIZE, 0, MAX_BUFF_SIZE);
		if (ret != 0) {
			cam_err("memset_s failed %s()%d", __func__, __LINE__);
			return;
		}
		ret = snprintf_s(buff, MAX_BUFF_SIZE,
			MAX_BUFF_SIZE - 1, "%s, fail times: %d.",
			g_fpga_common_err_table[SELF_TEST_ERR].err_name,
			drv_data->self_check_fail_time);
		if (ret < 0) {
			cam_err("snprintf_s failed %s()%d", __func__, __LINE__);
			return;
		}
		...
		fpga_dsm_client_notify(drv_data, buff,
			g_fpga_common_err_table[SELF_TEST_ERR].err_num);
	}
}
	...
	u8 addr, u8 *value, u8 limitVal)
{
	...
	cam_info("%s reg = 0x%x, val = 0x%x, limitVal = %d",
		__func__, addr, val, limitVal);

	reg_value = (val >> 4) * 100 + (val & 0x0F) * 8;
	if (reg_value > (limitVal * 100))
		return 1;
...
{
	struct ice40_spi_priv_data *drv_data = g_spi_drv_data;
	...
	cam_info("%s val = %d, g_ice40_enable_gpio_powerdown = %d",
		__func__, val, g_ice40_enable_gpio_powerdown);
	...
	if (val) {
		...
		mutex_lock(&g_gpio_power_ctrl_lock);
		g_color_set_gpio185_powerup = true;
		mutex_unlock(&g_gpio_power_ctrl_lock);
	} else {
		mutex_lock(&g_gpio_power_ctrl_lock);
		g_color_set_gpio185_powerup = false;
		mutex_unlock(&g_gpio_power_ctrl_lock);
		if (g_ice40_enable_gpio_powerdown)
...
int ice40_spi_exit(void)
{
	...
	drv_data->power_number--;
	cam_info("%s drv_data->power_number = %d", __func__,
		drv_data->power_number);
	...
	/* disable gpio */
	mutex_lock(&g_gpio_power_ctrl_lock);
	if (g_color_set_gpio185_powerup != true) {
		cam_info("%s g_color_set_gpio185_powerup = %d, make gpio185 pull down",
			__func__, g_color_set_gpio185_powerup);
		gpio_direction_output(plat_data->enable_gpio, 0);
	}
	mutex_unlock(&g_gpio_power_ctrl_lock);

	cam_info("%s drv_data->power_up_times = %d",
		__func__, drv_data->power_up_times);
	if (g_ice40_enable_gpio_powerdown != true) {
		if (drv_data->power_up_times >= POWERUP_MAX_TIME) {
			cam_info("%s drv_data->power_up_times = %d, will support power down",
				__func__, drv_data->power_up_times);
			g_ice40_enable_gpio_powerdown = true;
		}
	}
...
int ice40_spi_enable(void)
{
	...
	if (ret) {
		cam_err("%s read reg = 0x%x error, ret = %d",
			__func__, FPGA_IRQ_STATE_REG, ret);
		return ret;
	}
...
int ice40_spi_disable(void)
{
	...
	if (fpga_plus_wide_value_check(drv_data,
		FPGA_M1_PLUS_WIDE_REG, &val, REG_E_LIMIT_VAL) > 0) {
		cam_err("%s write reg = 0x%x error, ret = %d",
			__func__, FPGA_M1_PLUS_WIDE_REG, ret);
		fpga_dsm_client_notify(drv_data,
			g_fpga_common_err_table[AGE_PROJ_ERR].err_name,
			g_fpga_common_err_table[AGE_PROJ_ERR].err_num);
	}
	if (fpga_plus_wide_value_check(drv_data,
		FPGA_M2_PLUS_WIDE_REG, &val, REG_F_LIMIT_VAL) > 0) {
		cam_err("%s write reg = 0x%x error, ret = %d",
			__func__, FPGA_M2_PLUS_WIDE_REG, ret);
		fpga_dsm_client_notify(drv_data,
			g_fpga_common_err_table[AGE_ILLU_ERR].err_name,
			g_fpga_common_err_table[AGE_ILLU_ERR].err_num);
	}
	...
	if (ret)
		cam_err("%s write reg = 0x%x error, ret = %d",
			__func__, FPGA_STATE_CTRL1_REG, ret);
...
int ice40_spi_init_fun(void)
{
	...
	g_ice40_enable_gpio_powerdown = false;
	return 0;
}
	...
	u8 addr, u8 *val)
{
	...
	cam_info("%s devdata->rx_buf[0] = 0x%x, devdata->rx_buf[1] = 0x%x",
		__func__, devdata->rx_buf[0], devdata->rx_buf[1]);
...
static int ice40_spi_self_check(struct ice40_spi_priv_data *devdata)
{
	...
	if (ret) {
		cam_err("[%s][%d] ice40_spi_read_reg error, ret = %d",
	...
	cam_info("%s reg = 0x%x, val = 0x%x",
		__func__, FPGA_SELF_CHECK_REG, val);
...
static int ice40_spi_load_fw_code(struct ice40_spi_priv_data *devdata,
	char *name)
{
	...
	pos = fp->f_pos;
	for (left = file_size, block = 0; left > 0;
		left -= SPI_BLOCK_BUF_SIZE) {
		...
		ret = spi_write(devdata->spi, devdata->tx_buf,
			SPI_BLOCK_BUF_SIZE);
		if (ret < 0) {
			cam_err("%s spi send error, ret = %d, %d bytes, test",
				__func__, ret, one_size);
			break;
		}
		block++;
	}
...
int ice40_spi_load_fw(void)
{
	...
	if (drv_data->load_fw_state != LOAD_FW_NOT_START) {
		cam_info("%s load_fw_state = %d, will exit", __func__,
			drv_data->load_fw_state);
		return 0;
	}
...
static void fpga_parse_dmd_error(struct work_struct *work)
{
	...
	for (index = 0; index < ARRAY_SIZE(g_fpga_irq_err_table); index++) {
		if (err & g_fpga_irq_err_table[index].err_head) {
			cam_info("%s: index = %d", __func__, index);
			fpga_dsm_client_notify(drv_data,
				g_fpga_irq_err_table[index].err_name,
				g_fpga_irq_err_table[index].err_num);
			find = true;
		}
	}
	...
	if (find == false) {
		...
		fpga_dsm_client_notify(drv_data,
			g_fpga_common_err_table[OTHER_FILL_ERR].err_name,
			g_fpga_common_err_table[OTHER_FILL_ERR].err_num);
	}
}

static irqreturn_t fpga_irq_thread(int irq, void *handle)
{
	struct ice40_spi_priv_data *drv_data =
		(struct ice40_spi_priv_data *)handle;
	u8 val = 0x00;
	int ret;
	...
	if (drv_data->load_fw_state != LOAD_FW_DONE) {
		ret =  ice40_spi_read_reg(drv_data, FPGA_IRQ_STATE_REG, &val);
		cam_info("%s load_fw_state: %d, reg = 0x02, val = 0x%02x, ret: %d",
			__func__, drv_data->load_fw_state, val, ret);
...
#ifdef FPGA_READ_REG_DEBUG
static int hw_fpga_param_check(char *buf,
	unsigned long *param,
	int num_of_par)
{
	char *token = NULL;
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret =  ice40_spi_read_reg(drv_data, g_reg, &val);
	if (ret)
		cam_err("%s ice40_spi_read_reg error, ret = %d", __func__, ret);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cam_info("%s g_reg = %d g_reg_value = %d", __func__,
		g_reg, g_reg_value);

	if (g_reg_value != 0xFF)
		ice40_spi_write_reg(drv_data, g_reg, g_reg_value);
	else
...
static struct device_attribute g_dev_attr_fpga =
...
static int ice40_spi_get_dt_data(struct device *dev,
...
static int ice40_spi_probe(struct spi_device *spi)
{
	...
	drv_data->spi = spi;
	mutex_init(&g_gpio_power_ctrl_lock);
	...
	drv_data->fpga_clkout =
		devm_clk_get(&spi->dev, plat_data->fpga_clkname);
	...
	if (ret < 0) {
		cam_err("%s failed set_rate fpga_clkout rate", __func__);
		goto err_spi_setup;
	}
...
#ifdef FPGA_READ_REG_DEBUG
	ret = register_camerafs_attr(&g_dev_attr_fpga);
	if (ret < 0)
...
static const struct of_device_id g_ice40_spi_dt_ids[] = {
...
static struct spi_driver g_ice40_spi_drv = {
	...
	.driver = {
		...
		.owner =  THIS_MODULE,
		.bus =    &spi_bus_type,
		.of_match_table = g_ice40_spi_dt_ids,
	},
};

module_spi_driver(g_ice40_spi_drv);
