...
static BASP_LEVEL_TYPE get_basp_level(struct smartstar_coul_device *di)
{
	...
	if (!di->basp_batt_matched || di->basp_need_delay) {
		coul_core_info(BASP_TAG"skip basp, batt_matched %d,"
			"need_delay %d, force to LEVEL_0\n",
...
int get_fcc_after_cv_adjust(struct smartstar_coul_device *di, int fcc_mah)
{
	...
	if (!di->basp_total_level && di->nondc_volt_dec > BASP_FATAL_VDEC_TH) {
		delta_cv_pc = basp_full_pc_by_voltage(di) / TENTH;
		if (!delta_cv_pc)
			delta_cv_pc = PERCENT;
...
void hisi_coul_notify(struct smartstar_coul_device *di, enum hisi_coul_event event)
{
	...
	case HISI_OCV_UPDATE:
		...
		data.cc_value = di->iscd->full_update_cc;
