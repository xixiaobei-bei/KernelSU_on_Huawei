...
{
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 69))
	return dw_pcie_link_up(&pcie->pp);
#else
...
int kirin_pcie_enumerate(u32 rc_idx)
{
	...
	kirin_pcie_writel_rc(pp, 0, val + ((pcie->rc_id) << PCIE_DEV_ID_SHIFT));
#else
...
#ifdef CONFIG_KIRIN_PCIE_MAR
int kirin_pcie_bypass_pm(void)
{
	return (g_kirin_pcie[0].dtsinfo.ep_device_type == EP_DEVICE_MODEM) ? 1 : 0;
}
