...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 69))
static void set_atu_addr(struct pcie_port *pp, int type, u32 iatu_offset,
			u64 src_addr, u64 dst_addr, u32 size)
{
	kirin_pcie_writel_rc(pp, iatu_offset + KIRIN_PCIE_ATU_LOWER_BASE, lower_32_bits(src_addr));
	kirin_pcie_writel_rc(pp, iatu_offset + KIRIN_PCIE_ATU_UPPER_BASE, upper_32_bits(src_addr));
	kirin_pcie_writel_rc(pp, iatu_offset + KIRIN_PCIE_ATU_LIMIT, lower_32_bits(src_addr + size - 1));
	kirin_pcie_writel_rc(pp, iatu_offset + KIRIN_PCIE_ATU_LOWER_TARGET, lower_32_bits(dst_addr));
	kirin_pcie_writel_rc(pp, iatu_offset + KIRIN_PCIE_ATU_UPPER_TARGET, upper_32_bits(dst_addr));
	kirin_pcie_writel_rc(pp, iatu_offset + KIRIN_PCIE_ATU_CR1, type);
	kirin_pcie_writel_rc(pp, iatu_offset + KIRIN_PCIE_ATU_CR2, (u32)KIRIN_PCIE_ATU_ENABLE);//lint !e648
}
#else
			...
			u64 src_addr, u64 dst_addr, u32 size)
{
	...
	kirin_pcie_write_dbi(pci, pci->dbi_base,
			iatu_offset + KIRIN_PCIE_ATU_CR2, 0x4, (u32)KIRIN_PCIE_ATU_ENABLE);//lint !e648
}
#endif
		...
		int type, u64 src_addr, u64 dst_addr, u32 size)
{
	...
	if (iatu_offset != KIRIN_PCIE_ATU_VIEWPORT) {
		...
		if (direct & KIRIN_PCIE_ATU_REGION_INBOUND)//lint !e648
			iatu_offset += INBOUNT_OFFSET;
	} else
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 69))
		kirin_pcie_writel_rc(&pcie->pp, KIRIN_PCIE_ATU_VIEWPORT, (index | direct));//lint !e648

	set_atu_addr(&pcie->pp, type, iatu_offset, src_addr, dst_addr, size);
#else
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 69))
	val = kirin_pcie_readl_rc(&pcie->pp, iatu_offset + KIRIN_PCIE_ATU_CR2);
	val |= (msg_code | INHIBIT_PAYLOAD);
	kirin_pcie_writel_rc(&pcie->pp, iatu_offset + KIRIN_PCIE_ATU_CR2, val);

	writel(0x0, pcie->pp.dbi_base + MSG_CPU_ADDR);
#else
		...
		int type, u64 cpu_addr, u64 pci_addr, u32 size)
{
	...
	pcie = &g_kirin_pcie[rc_id];
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 69))
	kirin_pcie_atu_cfg(pcie, (u32)index, KIRIN_PCIE_ATU_REGION_OUTBOUND,
