...
static inline int occupy_free_smc_in_entry(tc_ns_smc_cmd *cmd)
{
	...
	for (i = 0; i < MAX_SMC_CMD; i++) {
		if (test_bit(i, g_cmd_data->in_bitmap))
			continue;
		if (memcpy_s(&g_cmd_data->in[i], sizeof(*cmd),
			cmd, sizeof(*cmd)) != EOK) {
...
static inline int copy_smc_out_entry(uint32_t idx, tc_ns_smc_cmd *copy,
	enum CMD_REUSE *usage)
{
	 ...
	 */
	if (g_cmd_data->out[idx].cmd_id == SS_AGENT_MSG ||
		g_cmd_data->out[idx].cmd_id == MT_AGENT_MSG) {
		if (usage != NULL)
			*usage = RESERVE;
	} else if (g_cmd_data->out[idx].ret_val == TEEC_PENDING2 ||
...
static inline int copy_shadow_task_agent_cmd(tc_ns_smc_cmd *copy)
{
	...
	acquire_smc_buf_lock(&g_cmd_data->smc_lock);
	for (i = 0; i < MAX_SMC_CMD; i++) {
		check_value = test_bit(i, g_cmd_data->out_bitmap) &&
			(g_cmd_data->out[i].ret_val == TEEC_PENDING2 &&
			g_cmd_data->out[i].cmd_id & (SHADOW_AGENT_FLAG));
		if (check_value == true) {
			if (memcpy_s(copy, sizeof(*copy), &g_cmd_data->out[i],
				sizeof(g_cmd_data->out[i]))) {
				tloge("copy smc out failed\n");
				release_smc_buf_lock(&g_cmd_data->smc_lock);
				return -1;
			}
			clear_bit(i, g_cmd_data->in_bitmap);
			clear_bit(i, g_cmd_data->doing_bitmap);
			clear_bit(i, g_cmd_data->out_bitmap);
			release_smc_buf_lock(&g_cmd_data->smc_lock);
			return 0;
		}
	}
...
void show_cmd_bitmap(void)
{
	...
	int cmd_in[MAX_SMC_CMD] = {-1};
	int cmd_out[MAX_SMC_CMD] = {-1};
	uint32_t in = 0;
	uint32_t out = 0;
...
static int shadow_wo_pm(u64 x0, u64 x1, u64 x2, u64 x3, u64 x4,
	bool *agent_request, int *n_idled, tc_ns_smc_cmd *cmd,
...
retry_wo_pm:
	ret_val = shadow_wo_pm(x0, x1, x2, x3, x4, &agent_request, &n_idled,
		&cmd, &ret, &exit_reason, &ta, &target);
	if (ret_val == -1)
		goto clean;
...
static void set_drm_strategy(void)
{
	if (!g_mask_flag) {
		...
		cpumask_set_cpu(4, &g_cpu_mask);
		cpumask_set_cpu(5, &g_cpu_mask);
		cpumask_set_cpu(6, &g_cpu_mask);
		cpumask_set_cpu(7, &g_cpu_mask);
		g_mask_flag = 1;
	}
...
int smc_init_data(struct device *class_dev)
{
	...
	if (class_dev == NULL || IS_ERR(class_dev))
		return -ENOMEM;
	g_cmd_data = (tc_ns_smc_queue *)__get_free_page(GFP_KERNEL | __GFP_ZERO);
	if (g_cmd_data == NULL)
		return -ENOMEM;
	...
	if (unlikely(IS_ERR(g_siq_thread))) {
...
free_siq_worker:
	...
	g_siq_thread = NULL;
free_mem:
	free_page((unsigned long)g_cmd_data);
	g_cmd_data = NULL;
...
int init_smc_svc_thread(void)
{
	g_smc_svc_thread = kthread_create(smc_svc_thread_fn, NULL,
		"smc_svc_thread");
	if (unlikely(IS_ERR(g_smc_svc_thread))) {
