...
int hisijpu_irq_request(struct hisi_jpu_data_type *hisijd)
{
	...
	if (hisijd->is_irq_merge) {
		if (hisijd->jpu_merged_irq) {
			ret = request_irq(hisijd->jpu_merged_irq, hisi_jpu_dec_merged_isr,
				0, IRQ_JPU_DEC_MERGED_NAME, (void *)hisijd);
			if (ret != 0) {
...
int hisi_jpu_register(struct hisi_jpu_data_type *hisijd)
{
	...
	if (!hisijd->jpgdDrv) {
		HISI_JPU_ERR("Failed: hipp_register.\n");
	}
...
int hisi_jpu_unregister(struct hisi_jpu_data_type *hisijd)
{
	...
	if (hipp_unregister(HISI_JPEGD_UNIT)) {
		HISI_JPU_ERR("Failed: hipp_unregister.\n");
	}
...
void hisi_jpu_dec_normal_reset(struct hisi_jpu_data_type *hisijd)
{
	...
	if(hisijd->jpu_support_platform == HISI_DSS_V501
		|| hisijd->jpu_support_platform == HISI_DSS_V510
		|| hisijd->jpu_support_platform == HISI_DSS_V510_CS) {
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_8, 0x1, 1, 25);
		ret = hisijpu_check_reg_state(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_8, 0x01000000);
		if (ret) {
			HISI_JPU_ERR("fail to wait JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_8!\n");
		}

		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_8, 0x0, 1, 25);
	} else {
...
void hisi_jpu_dec_error_reset(struct hisi_jpu_data_type *hisijd)
{
	...
	if(hisijd->jpu_support_platform == HISI_DSS_V501
		...
		|| hisijd->jpu_support_platform == HISI_DSS_V510_CS) {
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_8, 0x1, 1, 25);
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_9, 0x1, 1, 25);
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_8, 0x1, 1, 25);
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_9, 0x1, 1, 25);
	} else {
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_1, 0x1, 1, 25);
	...
	/* step2 */
	if(hisijd->jpu_support_platform == HISI_DSS_V510_CS) {
		ret = hisijpu_check_reg_state(hisijd->jpu_top_base + JPGDEC_PREF_STOP, 0x10);
		if(ret) {
			HISI_JPU_ERR("fail to wait JPGDEC_PREF_STOP!\n");
		}
	} else {
	...
	if(hisijd->jpu_support_platform == HISI_DSS_V501
		...
		ret = hisijpu_check_reg_state(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_9, 0x01000000);
		if(ret) {
			HISI_JPU_ERR("fail to wait JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_9!\n");
		}

		ret = hisijpu_check_reg_state(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_8, 0x01000000);
		if(ret) {
			HISI_JPU_ERR("fail to wait JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_8!\n");
		}

		ret = hisijpu_check_reg_state(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_9, 0x01000000);
		if(ret) {
			HISI_JPU_ERR("fail to wait JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_9!\n");
		}
	} else {
		ret = hisijpu_check_reg_state(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_1, 0x01000000);
		if(ret) {
	...
	if(hisijd->jpu_support_platform == HISI_DSS_V501
		...
		|| hisijd->jpu_support_platform == HISI_DSS_V510_CS) {
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_8, 0x0, 1, 25);
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_9, 0x0, 1, 25);
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_8, 0x0, 1, 25);
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_9, 0x0, 1, 25);
	} else {
		hisijpu_set_reg(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_1, 0x0, 1, 25);
...
static int hisi_jpu_dec_reg_default(struct hisi_jpu_data_type *hisijd)
{
	...
	if(hisijd->jpu_support_platform == HISI_DSS_V501
		|| hisijd->jpu_support_platform == HISI_DSS_V510
		|| hisijd->jpu_support_platform == HISI_DSS_V510_CS) {
		jpu_dec_reg->bitstreams_start_h = inp32(jpu_dec_base + JPEG_DEC_BITSTREAMS_START_H);
		jpu_dec_reg->bitstreams_start = inp32(jpu_dec_base + JPEG_DEC_BITSTREAMS_START_L);
		jpu_dec_reg->bitstreams_end_h = inp32(jpu_dec_base + JPEG_DEC_BITSTREAMS_END_H);
		jpu_dec_reg->bitstreams_end = inp32(jpu_dec_base + JPEG_DEC_BITSTREAMS_END_L);
	} else {
...
static int hisi_jpu_dec_set_cvdr(struct hisi_jpu_data_type *hisijd)
{
	...
	if(hisijd->jpu_support_platform == HISI_DSS_V501
		...
		outp32(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_8, 0x80060000);
		outp32(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_WR_CFG_9, 0x80060000);
		...
		/* NRRD1 */
		outp32(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_LIMITER_NR_RD_8, 0xf002222);
		outp32(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_8, 0x80060080);

		/* NRRD2 */
		outp32(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_LIMITER_NR_RD_9, 0xf003333);
		outp32(hisijd->jpu_cvdr_base + JPGDEC_CVDR_AXI_JPEG_NR_RD_CFG_9, 0x80060080);
...
static int hisi_jpu_dec_set_reg(struct hisi_jpu_data_type *hisijd, jpu_dec_reg_t *jpu_dec_reg)
{
	...
	if(hisijd->jpu_support_platform == HISI_DSS_V501
		|| hisijd->jpu_support_platform == HISI_DSS_V510
		|| hisijd->jpu_support_platform == HISI_DSS_V510_CS) {
		outp32(jpu_dec_base + JPEG_DEC_BITSTREAMS_START_H, jpu_dec_reg->bitstreams_start_h);
		outp32(jpu_dec_base + JPEG_DEC_BITSTREAMS_START_L, jpu_dec_reg->bitstreams_start);
		outp32(jpu_dec_base + JPEG_DEC_BITSTREAMS_END_H, jpu_dec_reg->bitstreams_end_h);
		outp32(jpu_dec_base + JPEG_DEC_BITSTREAMS_END_L, jpu_dec_reg->bitstreams_end);
	} else {
...
err_out:
	if (hisijd->jpgdDrv && hisijd->jpgdDrv->unlock_usecase){
		...
		ret1 = hisijd->jpgdDrv->unlock_usecase(hisijd->jpgdDrv);
		if (ret1 != 0) {
