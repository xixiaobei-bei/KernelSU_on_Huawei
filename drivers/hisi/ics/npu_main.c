...
int npu_work_get(unsigned int coreid)
{
    ...
    if (dev) {
        ...
        NPU_DEBUG("core%d, links_count=%d, usage_count=%d", coreid, dev->power.links_count,
...
static long npu_set_reset_virt_addr(unsigned long arg)
{
	...
	if (copy_to_user((void __user *)(uintptr_t)arg, &map_data, sizeof(map_data))) {
		if (ret == 0) {
			if (npu_smmu_cancel_reset_virt(&map_data))
				NPU_ERR("npu_smmu_unmap failed!\n");
		}
        NPU_ERR("copy_to_user failed!\n");
        ret = -EFAULT;
    }
...
static int npu_core_init(unsigned int coreid, struct platform_device *pdev)
{
    ...
    pm_runtime_init(&pdev->dev); // initialize the device run-time PM fields.
    pm_runtime_irq_safe(&pdev->dev); // make sure pm_runtime_put/get/ callable in irq context.
...
{
	NPU_DEBUG("call npu_pm_runtime_suspend enter!");
    unsigned int coreid = 0;
...
static int npu_pm_runtime_idle(struct device *dev)
{
    ...
    int ret = 0;
    if (dev == adapter_pm_core0->dev) {
        ret = pm_schedule_suspend(dev, NPU_SUSPEND_DELAY);
        if (ret != 0) {
            NPU_DEBUG("core0, links_count=%d, usage_count=%d, ret=%d", dev->power.links_count,
                atomic_read(&dev->power.usage_count), ret);
        ...
        ret = pm_schedule_suspend(dev, NPU_SUSPEND_DELAY);
        if (ret != 0) {
            NPU_DEBUG("core1, links_count=%d, usage_count=%d, ret=%d", dev->power.links_count,
                atomic_read(&dev->power.usage_count), ret);
            ret = pm_schedule_suspend(dev, 0);
        }
        NPU_DEBUG("core1, links_count=%d, usage_count=%d, ret=%d", dev->power.links_count,
