...
static void npu_mntn_write_peri_reg_info(unsigned int core_id)
{
	...
	int ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX + 1, NPU_BUF_LEN_MAX, "peri_stat=%x, ppll_select=%x, power_stat=%x, power_ack=%x, reset_stat=%x, perclken=%x, perstat=%x.\r\n",
			 peri_reg->peri_stat,
...
static void npu_mntn_write_mstr_reg_info(unsigned int core_id)
{
	...
	int ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX + 1, NPU_BUF_LEN_MAX, "RD_BITMAP=%x, WR_BITMAP=%x, rd_cmd_total_cnt[0-3]={%x, %x, %x}, wr_cmd_total_cnt=%x\n",
			mstr_reg->rd_bitmap,
			mstr_reg->wr_bitmap,
...
static void npu_mntn_write_reg_log(unsigned int core_id)
{
	...
	switch (g_mntn_private->mntn_info.dump_info.modid) {
		case MODID_NPU_EXC_DEAD:  //lint !e650
			ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX + 1, NPU_BUF_LEN_MAX, "interrupt_status=0x%x\r\n", g_mntn_private->exc_info[core_id].interrupt_status);
	        if (ret < 0) {
				NPU_ERR("snprintf_s failed\n");
				return;
			}
