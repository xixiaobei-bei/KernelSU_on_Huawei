...
int hisee_encos_header_init(void)
{
	...
	goto out;
first_init:
	(void)memset_s(&g_hisee_encos_header, sizeof(g_hisee_encos_header), 0x0, sizeof(g_hisee_encos_header));
	ret = memcpy_s((void *)g_hisee_encos_header.magic, HISEE_ENCOS_MAGIC_LEN,
					HISEE_ENCOS_MAGIC_VALUE, (unsigned long)HISEE_ENCOS_MAGIC_LEN);
	if (ret != EOK) {
		pr_err("%s(): memcpy_s err.\n", __func__);
		goto out;
	}
...
int hisee_encos_write(const char *data_buf, unsigned int size, unsigned int cos_id)
{
	...
	ret = strncpy_s(g_hisee_encos_header.file[file_id].name, HISEE_ENCOS_SUB_FILE_NAME_LEN,
					HISEE_IMG_COS_NAME, (unsigned long)HISEE_IMG_SUB_FILE_LEN);
	if (ret != EOK) {
