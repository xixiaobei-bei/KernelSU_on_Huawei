...
int cos_upgrade_image_read(unsigned int cos_id, hisee_img_file_type img_type)
{
	...
	if (COS_FLASH_IMG_ID == cos_id && COS_FLASH_IMG_TYPE == img_type) {
		ret = strncat_s(fullname, MAX_PATH_NAME_LEN,
						HISEE_FS_PARTITION_NAME, (MAX_PATH_NAME_LEN - 1) - strlen(fullname));
		if (ret != EOK) {
		...
		ret = strncat_s(fullname, MAX_PATH_NAME_LEN,
						HISEE_COS_FLASH_IMG_NAME, strlen(HISEE_COS_FLASH_IMG_NAME));
		if (ret != EOK) {
