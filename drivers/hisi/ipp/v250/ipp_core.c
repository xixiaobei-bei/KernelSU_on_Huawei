...
static int hispcpe_powerup(struct hispcpe_s *dev)
{
	int ret = 0, err = 0;
	...
	if (dev->cpe_supply_refs > 0) {
		pr_err("[%s] Failed: cpe_supply Opened\n", __func__);
		return 0;
	}
...
static int hispcpe_work_check(struct hispcpe_s *dev)
{
...
#define REQ_WAIT_TIME 5
#define REQ_WAIT_NUMS 10
static int hispcpe_work_check_wait(struct hispcpe_s *dev)
{
	int ret;
	int i;

	for (i = 0; i < REQ_WAIT_NUMS; i++) {
		ret = hispcpe_work_check(dev);
		if (ret == 0)
			break;
		msleep(REQ_WAIT_TIME);
	}
...
int hispcpe_powerdn(struct hispcpe_s *dev)
{
	...
	if (dev->cpe_supply_refs <= 0) {
		pr_err("[%s] Failed: cpe_supply no pu\n", __func__);
		return -EINVAL;
	}
...
int hispcpe_mcf_request(struct hispcpe_s *dev, msg_req_mcf_request_t *ctrl)
{
	...
	ret = mcf_request_handler(ctrl);
	if (ret != 0)
		pr_err("[%s] Failed : mcf_request_handler check.%d\n", __func__, ret);
...
int hispcpe_slam_request(struct hispcpe_s *dev, msg_req_slam_request_t *ctrl)
{
	...
	ret = slam_request_handler(ctrl);
	if(ret != 0) {
		pr_err("[%s] Failed : slam_request_handler.%d\n", __func__, ret);
		return ret;
	}
...
static int hispcpe_map_kernel_check_param(struct hispcpe_s *dev, struct memory_block_s *frame_buf)
{
	...
	if (dev->devbuf->size != frame_buf->size) {
		pr_err("[%s] Failed : dma buf get, size not equal\n", __func__);
...
static int hispcpe_map_kernel(struct hispcpe_s *dev, unsigned long args, unsigned int *ipp_buff_size)
{
	...
	if (dev->mapbuf_ready > 0) {
		pr_info("[%s] Failed : map_kernel already done\n", __func__);
		return -EINVAL;
	}
	...
	unsigned long args, struct memory_block_s *buf)
{
	...
	case HISP_CPE_PWRUP:
		...
		ret = copy_from_user(&para, args_pwup, sizeof(para));
		if (ret != 0) {
			pr_err("[%s] copy_from_user.%d\n", __func__, ret);
			return -EFAULT;
		}

		pr_info("%s: HISP_CPE_PWRUP.%d\n", __func__, para.pw_flag);
		if (para.pw_flag != 0) {
			pr_err("[%s] Failed:layer pw_flag invalid\n", __func__);
			return -EINVAL;
		}

		mutex_lock(&dev->dev_lock);
		ret = hispcpe_powerup(dev);
		if (ret != 0) {
...
static int hispcpe_release(struct inode *inode, struct file *filp)
{
	...
	ret = hispcpe_work_check_wait(dev);
	if (ret == 0) {
...
int ippdev_lock(void)
{
	...
	mutex_lock(&hispcpe_dev->cvdr_lock);
	d("-\n");
	return 0;
}
...
int ippdev_unlock(void)
{
	...
	mutex_unlock(&hispcpe_dev->cvdr_lock);
	d("-\n");
	return 0;
}
//lint -restore
