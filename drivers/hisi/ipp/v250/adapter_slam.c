...
static int slam_check_request_para(msg_req_slam_request_t *slam_request)
{
	unsigned int i = 0;
	unsigned int score_cnt = 0;
	unsigned int kpt_cnt = 0;
	unsigned int total_kpt_cnt = 0;

	for(i = 0; i < slam_request->slam_pyramid_layer; i++) {
		if((slam_request->req_orb.streams[i][BI_SLAM_Y].buf == 0) ||
			(slam_request->req_orb.streams[i][BO_SLAM_STAT].buf == 0))
			return CPE_FW_ERR;

		if(slam_request->req_orb.streams[i][BO_SLAM_SCORE_HIST].buf != 0)
			score_cnt += 1;

		if(slam_request->req_orb.streams[i][BO_SLAM_KPT_NUM].buf != 0)
			kpt_cnt += 1;

		if(slam_request->req_orb.streams[i][BO_SLAM_TOTAL_KPT].buf != 0)
			total_kpt_cnt += 1;
	}

	if((score_cnt != 0) && (score_cnt != slam_request->slam_pyramid_layer))
		return CPE_FW_ERR;

	if((kpt_cnt != 0) && (kpt_cnt != slam_request->slam_pyramid_layer))
		return CPE_FW_ERR;

	if((total_kpt_cnt != 0) && (total_kpt_cnt != slam_request->slam_pyramid_layer))
		return CPE_FW_ERR;
...
int slam_request_handler(msg_req_slam_request_t *slam_request)
{
	...
	ret = slam_check_request_para(slam_request);
	if(ret != CPE_FW_OK) {
		e("[%s] failed : slam_check_request_para", __func__);
		return CPE_FW_ERR;
	}
