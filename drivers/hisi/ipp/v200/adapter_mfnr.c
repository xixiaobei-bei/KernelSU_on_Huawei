...
static int mfnr_calc_stripe_info(unsigned int is_full_mode, cpe_stream_t *stream, isp_stripe_info_t *stripe_mfnr_origin,
								 isp_stripe_info_t *stripe_mfnr_ds4, isp_stripe_info_t *stripe_mfnr_ds16, isp_stripe_info_t *stripe_mfnr_gm);

static int mfnr_update_top_cfg_tab(msg_req_mfnr_request_t *mfnr_request, cpe_top_config_table_t *cfg_tab,
								   isp_stripe_info_t *stripe_mfnr_origin, isp_stripe_info_t *stripe_mfnr_ds4, isp_stripe_info_t *stripe_mfnr_gm,
								   unsigned int stripe_index)
{
	...
	if (mfnr_request->streams[BO_MFNR_RESULT_Y].buf) {
		...
		cfg_tab->crop_vpwr.crop_vpwr_1.vpwr_ihright = stripe_mfnr_origin->stripe_width[stripe_index] -
				stripe_mfnr_origin->overlap_right[stripe_index];
	} else {
	...
	if (mfnr_request->streams[BO_MFNR_RESULT_UV].buf) {
		...
		cfg_tab->crop_vpwr.crop_vpwr_0.vpwr_ihright = stripe_mfnr_ds4->stripe_width[stripe_index] -
				stripe_mfnr_ds4->overlap_right[stripe_index];
	} else {
	...
	if (mfnr_request->streams[BO_MFNR_GHOST_MASK].buf) {
		...
		cfg_tab->crop_vpwr.crop_vpwr_2.vpwr_ihright = stripe_mfnr_gm->stripe_width[stripe_index] -
				stripe_mfnr_gm->overlap_right[stripe_index];
	} else {
...
static int mfnr_update_cvdr_rd_cfg_tab(msg_req_mfnr_request_t *mfnr_request, cfg_tab_cvdr_t *cfg_tab,
									   isp_stripe_info_t *stripe_mfnr_origin, isp_stripe_info_t *stripe_mfnr_ds4, isp_stripe_info_t *stripe_mfnr_ds16,
									   isp_stripe_info_t *stripe_mfnr_gm, unsigned int stripe_index)
{
	D("[%s] +\n", __func__);
	cvdr_opt_fmt_t cfmt;
	...
	if (mfnr_request->streams[BI_MFNR_REF_MATCH_YUV_DS16].buf) {
		...
		if (mfnr_request->mfnr_cfg.op_mode == MFNR_FULL_COLOR_MODE) {
			...
			cfmt.addr = mfnr_request->streams[BI_MFNR_REF_MATCH_YUV_DS16].buf + (stripe_mfnr_ds16->stripe_start_point[stripe_index]
						* 6);
			...
			pr_info("cfmt.width=%d  cfmt.height=%d\n", cfmt.width, cfmt.height);
				   ...
				   || (mfnr_request->mfnr_cfg.op_mode == MFNR_PARTIAL_MONO_MODE)) {
			...
			cfmt.addr = mfnr_request->streams[BI_MFNR_REF_MATCH_YUV_DS16].buf +
						(stripe_mfnr_origin->stripe_start_point[stripe_index]);
...
static int mfnr_update_cvdr_wr_cfg_tab(msg_req_mfnr_request_t *mfnr_request, cfg_tab_cvdr_t *cfg_tab,
									   isp_stripe_info_t *stripe_mfnr_origin, isp_stripe_info_t *stripe_mfnr_ds4, isp_stripe_info_t *stripe_mfnr_ds16,
									   isp_stripe_info_t *stripe_mfnr_gm, unsigned int stripe_index)
{
	D("[%s] +\n", __func__);
	...
	if (mfnr_request->streams[BO_MFNR_RESULT_UV].buf) {
		...
		cfmt.full_width = stripe_mfnr_ds4->full_size.width;
		cfmt.pix_fmt = DF_FMT_INVALID;
		cfmt.addr = mfnr_request->streams[BO_MFNR_RESULT_UV].buf + (stripe_mfnr_ds4->stripe_start_point[stripe_index] +
					stripe_mfnr_ds4->overlap_left[stripe_index]) * 2;
	...
	if (mfnr_request->streams[BO_MFNR_RESULT_Y].buf) {
		...
		cfmt.full_width = stripe_mfnr_origin->full_size.width;
		cfmt.pix_fmt = DF_FMT_INVALID;
		cfmt.addr = mfnr_request->streams[BO_MFNR_RESULT_Y].buf + (stripe_mfnr_origin->stripe_start_point[stripe_index] +
					stripe_mfnr_origin->overlap_left[stripe_index]);
	...
	if (mfnr_request->streams[BO_MFNR_GHOST_MASK].buf) {
		...
		cfmt.pix_fmt = DF_FMT_INVALID;
		cfmt.addr = mfnr_request->streams[BO_MFNR_GHOST_MASK].buf + (stripe_mfnr_gm->stripe_start_point[stripe_index] +
					stripe_mfnr_gm->overlap_left[stripe_index]);
...
static int mfnr_update_mfnr_cfg_tab(unsigned int current_stripe, unsigned int stripe_cnt,
									msg_req_mfnr_request_t *mfnr_request, mfnr_config_table_t *cfg_tab, unsigned int stripe_width)
{
	D("[%s] +\n", __func__);
	cfg_tab->to_use = 1;
	...
	case MFNR_FULL_COLOR_MODE: {
		...
		cfg_tab->temporal_fusion.dghost_sigma_stren.max_grad2sigma_scale_bkpt =
			para.temporal_fusion_yuv.dghost_sigma_stren.max_grad2sigma_scale_bkpt;
		cfg_tab->temporal_fusion.dghost_sigma_stren.min_grad2sigma_scale_bkpt =
			para.temporal_fusion_yuv.dghost_sigma_stren.min_grad2sigma_scale_bkpt;
		cfg_tab->temporal_fusion.dghost_sigma_stren.judge_grad_region_th      =
			para.temporal_fusion_yuv.dghost_sigma_stren.judge_grad_region_th;
		cfg_tab->temporal_fusion.dghost_sigma_stren.sigma                     =
			para.temporal_fusion_yuv.dghost_sigma_stren.sigma;
		cfg_tab->temporal_fusion.dghost_blend_stren.max_grad2blend_scale_bkpt =
			para.temporal_fusion_yuv.dghost_blend_stren.max_grad2blend_scale_bkpt;
		cfg_tab->temporal_fusion.dghost_blend_stren.min_grad2blend_scale_bkpt =
			para.temporal_fusion_yuv.dghost_blend_stren.min_grad2blend_scale_bkpt;
		cfg_tab->temporal_fusion.dghost_blend_stren.sup_mean2blend_th         =
			para.temporal_fusion_yuv.dghost_blend_stren.sup_mean2blend_th;
		cfg_tab->temporal_fusion.dghost_blend_stren.sup_blend_scale           =
			para.temporal_fusion_yuv.dghost_blend_stren.sup_blend_scale;
		memcpy_s(cfg_tab->temporal_fusion.dghost_mean2sigma_lut, MFNR_MEAN2SIGMA * sizeof(unsigned int),
				 para.temporal_fusion_yuv.dghost_mean2sigma_lut, MFNR_MEAN2SIGMA * sizeof(unsigned int));
		memcpy_s(cfg_tab->temporal_fusion.dghost_grad2sigma_lut, MFNR_GRAD2SIGMA * sizeof(unsigned int),
				 para.temporal_fusion_yuv.dghost_grad2sigma_lut, MFNR_GRAD2SIGMA * sizeof(unsigned int));
		memcpy_s(cfg_tab->temporal_fusion.dghost_mean2blend_lut, MFNR_MEAN2BLEND * sizeof(unsigned int),
				 para.temporal_fusion_yuv.dghost_mean2blend_lut, MFNR_MEAN2BLEND * sizeof(unsigned int));
		memcpy_s(cfg_tab->temporal_fusion.dghost_grad2blend_lut, MFNR_GRAD2BLEND * sizeof(unsigned int),
				 para.temporal_fusion_yuv.dghost_grad2blend_lut, MFNR_GRAD2BLEND * sizeof(unsigned int));
		break;
	}
	...
	case MFNR_FULL_MONO_MODE: {
		...
		cfg_tab->temporal_fusion.dghost_blend_stren.sup_blend_scale           =
			para.temporal_fusion_yuv.dghost_blend_stren.sup_blend_scale;
		memcpy_s(cfg_tab->temporal_fusion.dghost_mean2sigma_lut, MFNR_MEAN2SIGMA * sizeof(unsigned int),
				 para.temporal_fusion_yuv.dghost_mean2sigma_lut,
				 MFNR_MEAN2SIGMA * sizeof(unsigned int)); // unsafe_function_ignore: memcpy
		memcpy_s(cfg_tab->temporal_fusion.dghost_grad2sigma_lut, MFNR_GRAD2SIGMA * sizeof(unsigned int),
				 para.temporal_fusion_yuv.dghost_grad2sigma_lut,
				 MFNR_GRAD2SIGMA * sizeof(unsigned int)); // unsafe_function_ignore: memcpy
		memcpy_s(cfg_tab->temporal_fusion.dghost_mean2blend_lut, MFNR_MEAN2BLEND * sizeof(unsigned int),
				 para.temporal_fusion_yuv.dghost_mean2blend_lut,
				 MFNR_MEAN2BLEND * sizeof(unsigned int)); // unsafe_function_ignore: memcpy
		memcpy_s(cfg_tab->temporal_fusion.dghost_grad2blend_lut, MFNR_GRAD2BLEND * sizeof(unsigned int),
				 para.temporal_fusion_yuv.dghost_grad2blend_lut,
				 MFNR_GRAD2BLEND * sizeof(unsigned int)); // unsafe_function_ignore: memcpy
		break;
	}
...
static void mfnr_update_cmdlst_cfg_tab(cfg_tab_cmdlst_t *cmdlst_cfg, unsigned int channel_id,
...
static int mfnr_set_irq_mode(msg_req_mfnr_request_t *mfnr_request, schedule_cmdlst_link_t *cmd_link_entry,
...
int mfnr_request_handler(msg_req_mfnr_request_t *mfnr_request)
{
	...
	unsigned int is_full_mode = (mfnr_request->mfnr_cfg.op_mode == MFNR_FULL_COLOR_MODE
								 || mfnr_request->mfnr_cfg.op_mode == MFNR_FULL_MONO_MODE) ? 1 : 0;
	...
	mfnr_calc_stripe_info(is_full_mode, &mfnr_request->streams[BI_MFNR_CURR_Y], stripe_mfnr_origin, stripe_mfnr_ds4,
						  stripe_mfnr_ds16, stripe_mfnr_gm);
	...
	for (i = 0; i < stripe_cnt; i++) {
		...
		mfnr_update_cvdr_wr_cfg_tab(mfnr_request, &cvdr_cfg_tab[i], stripe_mfnr_origin, stripe_mfnr_ds4, stripe_mfnr_ds16,
									stripe_mfnr_gm, i);
		mfnr_update_cvdr_rd_cfg_tab(mfnr_request, &cvdr_cfg_tab[i], stripe_mfnr_origin, stripe_mfnr_ds4, stripe_mfnr_ds16,
									stripe_mfnr_gm, i);
