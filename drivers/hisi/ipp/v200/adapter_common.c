...
void cmdlst_update_buffer_header(cmd_buf_t *cmd_buf, unsigned int channel_id)
{
	...
	*(unsigned int *)(cmd_buf->start_addr + 0x4) = (next_hw_prio << 31) | (next_hw_resource << 8) |
...
int df_cmdlst_config_vpwr(cmd_buf_t *cmd_buf, unsigned int stripe_index, unsigned int  fs)
{
	...
	unsigned int err = 0;
	err = memset_s(&cmdlst_w3_1_table, sizeof(cfg_tab_cvdr_t), 0, sizeof(cfg_tab_cvdr_t));

	if (err != 0) {
		pr_err(" fail to memset_s\n");
		return CPE_FW_ERR;
	}
	...
	cmdlst_w3_1_table.vp_wr_cfg[WR_CMDLST].fmt.last_page = (cmdlst_w3_1_table.vp_wr_cfg[WR_CMDLST].fmt.fs_addr + 0x10000) >>
			15;
...
static int cmdlst_set_vpwr(cmd_buf_t *cmd_buf, cmdlst_rd_cfg_info_t *rd_cfg_info)
{
	...
	unsigned int err = memset_s(&cmdlst_w3_1_table, sizeof(cfg_tab_cvdr_t), 0, sizeof(cfg_tab_cvdr_t));
...
int df_sched_prepare(cmdlst_para_t *cmdlst_para)
{
	...
	unsigned int err = memset_s(new_entry, MAX_STRIPE_NUM * sizeof(schedule_cmdlst_link_t), 0,
...
void df_size_dump_stripe_info(isp_stripe_info_t *p_stripe_info, char *s)
{
	...
	err = memset_s((void *)type_name, LOG_NAME_LEN, 0, LOG_NAME_LEN);
	...
	err = memset_s((void *)name, sizeof(name), 0, sizeof(name));
