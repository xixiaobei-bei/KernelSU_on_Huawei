                                    ...
                                    loff_t *off)
{
    ...
    if (is_ivp_in_secmode()) {
        ...
        if ((CMD_ALGORUN == ((unsigned char)tmp_buff[3] & 0x7F)) && (0 == tmp_buff[0])) {
            ret = ivp_trans_sharefd_to_phyaddr(&pdev->ipc_pdev->dev, (unsigned int *)tmp_buff);
            if (ret < 0) {
    ...
    if (0 == tmp_buff[0])
    {
        ivp_info("receive ipc cmd 0x%x",((unsigned char)tmp_buff[3] & 0x7F));
    }
