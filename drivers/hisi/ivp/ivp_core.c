...
void *ivp_vmap(phys_addr_t paddr, size_t size, unsigned int * offset)
{
    ...
    *offset = paddr & ~PAGE_MASK;
    ...
    for (i = 0; i < pages_count; i++) {
        *(pages + i) = phys_to_page((phys_addr_t)(paddr + PAGE_SIZE * i));
    }
...
static int ivp_poweron(struct ivp_device *pdev, const struct ivp_power_up_info* pu_info)
{
    int ret = 0;
    ...
    if ((SECURE_MODE == pu_info->sec_mode)&&(0 == pdev->ivp_sec_support)){
...
static long ivp_ioctl(struct file *fd, unsigned int cmd, unsigned long args)
{
    ...
    case IVP_IOCTL_POWER_UP:{
        struct ivp_power_up_info info;
        if (!args) {
            ivp_err("Invalid input args 0");
            return -EINVAL;
        }
        mutex_lock(&ivp_power_up_off_mutex);
        if (0 != copy_from_user(&info, (void *)(uintptr_t)args, sizeof(info))) {
    ...
    case IVP_IOCTL_SECTINFO: {
        ...
        if (SECURE_MODE == ivp_dev.ivp_secmode){
            ret = copy_to_user((void *)(uintptr_t)args, &ivp_dev.sec_sects[info.index], sizeof(struct ivp_sect_info));

        }
        else
        {
            ret = copy_to_user((void *)(uintptr_t)args, &ivp_dev.sects[info.index], sizeof(struct ivp_sect_info));
        }
    ...
    case IVP_IOCTL_LOAD_FIRMWARE: {
        ...
        if (rc != EOK) {
            ivp_err("(%s):memcpy_s fail, ret [%d]",__FUNCTION__,rc);
    ...
    case IVP_IOCTL_CLK_LEVEL: {
        ...
        if (0 != copy_from_user(&level, (void *)(uintptr_t)args, sizeof(unsigned int))) {
            ivp_err("Invalid input param size.");
            return -EINVAL;
        }
    ...
    case IVP_IOCTL_DUMP_DSP_STATUS: {
