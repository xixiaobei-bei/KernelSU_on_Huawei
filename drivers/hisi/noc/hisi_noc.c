...
NOC_ERROR_CHECK:
					if (is_noc_node_available(node)) {
						...
						if (i < MAX_MODID_NUM) {
							pr_err("[%s] node->modid = %x!\n", __func__, node->noc_modid);
							temp_modid[i++] = node->noc_modid;
						}
					} else {
...
static void get_noc_node_clock(struct device_node *np, struct noc_node *node)
{
	...
	if (EOK != memset_s(&node->crg_clk[0], 
		sizeof(struct noc_clk) * HISI_NOC_CLOCK_MAX, 0xff, sizeof(struct noc_clk) * HISI_NOC_CLOCK_MAX)) {
...
static int hisi_noc_get_property(struct device_node *np)
{
	...
	if (0 == of_property_read_u32_array(np,
		...
		if (EOK != memcpy_s(&noc_property_dt.apcu_init_flow_array,
				sizeof(dts_u32_array), dts_u32_array, sizeof(dts_u32_array))) {
