...
bool emcom_xengine_mpflow_checkvalid(struct sock *sk, struct sockaddr *uaddr, int8_t index, uint16_t *dport)
{
	...
	if (!(((sk->sk_protocol == IPPROTO_TCP) &&
		...
		EMCOM_LOGD("mpflow check valid protocol not correct uid: %u, sk: %p",
...
void emcom_xengine_mpflow_bind2device(struct sock *sk, struct sockaddr *uaddr)
{
	...
	if ((!dev) || (emcom_xengine_mpflow_getinetaddr(dev) == false)) {
		rcu_read_unlock();
		EMCOM_LOGD("mpflow bind2device dev not ready uid: %u, sk: %p, dev: %p, name: %s",
	...
	EMCOM_LOGD("mpflow bind2device success uid: %u, sk: %p, "
			   "ifname: %s, ifindex: %d",
...
int8_t emcom_xengine_mpflow_checkstatus(struct sock *sk, int reason, int state, struct emcom_xengine_mpflow_stat *node)
{
	...
	if (reason == EMCOM_MPFLOW_FALLBACK_NOPAYLOAD) {
		...
		if ((oldstate == TCP_ESTABLISHED) && (state != TCP_ESTABLISHED)) {
			result = ((tp->bytes_received <= 1) && (tp->bytes_acked > 1)) ? EMCOM_MPFLOW_FALLBACK_SET : EMCOM_MPFLOW_FALLBACK_CLR;
	...
	if (result != EMCOM_MPFLOW_FALLBACK_NONE) {
		EMCOM_LOGD("MpFlow checkinfo uid:%d sk:%p inf:%d(%s) R:%d P(%d->%d) snt,ack,rcv:(%d):%u,%llu,%llu\n",
				   node->uid, sk, node->ifindex, node->name, reason, ntohs(inet->inet_sport), ntohs(inet->inet_dport),
				   result, tp->segs_out, tp->bytes_acked, tp->bytes_received);
	}
...
static void emcom_xengine_mpflow_reset(struct tcp_sock *tp)
{
	...
	EMCOM_LOGD("reset sk %p sport is %u", tp, sk->sk_num);
...
static void emcom_xengine_mpflow_download_flow_add(struct sock *sk)
{
	...
	if (index != INDEX_INVALID) {
		...
		if (node) {
			...
			EMCOM_LOGD("sk %p is a download flow sport %u ", tp, port);
