...
static struct wakeup_data *pdata;
...
static long wakeup_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if (pdata == NULL){
        printk("%s: wakeup ioctl error: pdata = null!\n", __func__);
        return -EBUSY;
    }
    ...
    switch (cmd) {
        ...
        case WAKEUP_INFO_EVENT:
            wakeInfo = (char *)arg;
            copy_from_user(envp_hal, wakeInfo, ENVP_LENTH);
            printk("%s: wakeup info event(%s).\n", __func__, envp_hal);
            ret = wakeup_notifier_call(1, envp_hal);
            break;
        default:
...
static struct miscdevice wakeup_miscdev = {
...
static int wakeup_notifier_call(unsigned long event, char *str)
{
    ...
    printk("%s: wakeup_notifier_call: wakeup = %lu, str = %s\n", __func__, event, str);
    ...
    kobject_uevent_env(&wakeup_miscdev.this_device->kobj, KOBJ_CHANGE, envp_ext);
    mutex_unlock(&pdata->notifier_lock);
...
static int wakeup_probe(struct platform_device *pdev)
{
    ...
    ret = misc_register(&wakeup_miscdev);
    if (ret != 0) {
...
err_out:
    misc_deregister(&wakeup_miscdev);
...
{
    int ret;
    ret = platform_driver_register(&wakeup_driver);
    if (ret) {
...
{
    platform_driver_unregister(&wakeup_driver);
}
