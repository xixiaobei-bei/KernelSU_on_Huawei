...
static int lcdkit_on(struct platform_device* pdev)
{
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
      ...
      LOG_JANK_D(JLID_KERNEL_LCD_POWER_ON, "%s", "LCD_POWER_ON");
		if (lcdkit_info.panel_infos.lcd_poweron_after_bl == 1) {
			/* set sched prio for jdn2 boe hx8279d */
      ...
      if ((!lcdkit_is_enter_sleep_mode()) && (!g_lcd_prox_enable))
      {
		...
		if (lcdkit_info.panel_infos.lcd_poweron_after_bl == 1) {
			LCDKIT_INFO("delay_af_vbat_on %d!\n",
				lcdkit_info.panel_infos.delay_af_vbat_on);
			usleep_range(lcdkit_info.panel_infos.delay_af_vbat_on * 1000,
				(lcdkit_info.panel_infos.delay_af_vbat_on + 1) * 1000);
		} else {
    ...
    {
		if (lcdkit_info.panel_infos.lcd_poweron_after_bl == 1)
			usleep_range(lcdkit_info.panel_infos.delay_af_LP11 * 1000,
				(lcdkit_info.panel_infos.delay_af_LP11 + 1) * 1000);
		else
...
lp_sequence_restart:
        ...
        else
        {
			...
			if (lcdkit_info.panel_infos.lcd_poweron_after_bl == 1)
				/* restore sched prio for jdn2 boe hx8279d */
...
static void vbat_powerdown_pwm(void)
{
	...
	if (strncmp(lcdkit_info.panel_infos.panel_name,
		"BOE_V2_HX8279 NEW 8P0 VIDEO TFT 1200 x 1920",
		strlen(lcdkit_info.panel_infos.panel_name)) == 0) {
