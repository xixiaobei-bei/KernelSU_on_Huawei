...
static int kbase_platform_device_remove(struct platform_device *pdev)
{
	...
	if (kbdev->hisi_dev_data.gpu_chip_type == 1 &&
		kbase_has_hi_feature(kbdev, KBASE_FEATURE_ENABLE_ARM_WA_JOB)) {
...
static int load_workaround(struct kbase_device *kbdev)
{
	...
	dev_err(kbdev->dev, "Loaded firmware of size %zu bytes. wa.kctx=%p\n",
		firmware->size, kctx);
	 ...
	 */
	kbdev->wa.flags = kbdev->wa.flags & ~KBASE_WA_FLAG_LOGICAL_SHADER_POWER;
#endif
