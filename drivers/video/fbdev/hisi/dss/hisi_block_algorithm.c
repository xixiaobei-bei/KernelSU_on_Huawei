...
static uint32_t calc_h_ratio(dss_overlay_t *pov_req)
{
	...
	if (pov_req->wb_compose_type == DSS_WB_COMPOSE_COPYBIT) {
		...
		if (pov_req->wb_layer_infos[0].transform == (HISI_FB_TRANSFORM_ROT_90 | HISI_FB_TRANSFORM_FLIP_V))
	...
	dss_layer_t *h_layer, dss_rect_t ov_block_rect, struct wb_block_layer_info *wb_blk)
{
	...
	if (pov_req_h_v->wb_compose_type == DSS_WB_COMPOSE_COPYBIT) {
		...
		if (pov_req_h_v->wb_layer_infos[0].transform == (HISI_FB_TRANSFORM_ROT_90 | HISI_FB_TRANSFORM_FLIP_V))
