		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (!priv->cdc_ctrl->pm_runtime_support) {
			...
			mdelay(2);
			ret = slimbus_switch_framer(SLIMBUS_DEVICE_HI6403, SLIMBUS_FRAMER_CODEC);
			if (ret) {
				pr_err("%s : slimbus switch framer to codec failed %d\n", __FUNCTION__, ret);
				ret = -EBUSY;
			}
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		if (!priv->cdc_ctrl->pm_runtime_support) {
			...
			ret = slimbus_switch_framer(SLIMBUS_DEVICE_HI6403, SLIMBUS_FRAMER_SOC);
			if (ret) {
				pr_err("%s : slimbus switch framer to soc failed %d\n", __FUNCTION__, ret);
				ret = -EBUSY;
			}
			mdelay(2);
		}
...
{
	char state = (unsigned int)classh_state_cfg & 0x0F;
...
int hi6403_classh_rcv_hp_switch(bool enable)
{
	...
	if (enable) {
		priv->rcv_hp_classh_state = (unsigned int)priv->rcv_hp_classh_state | rcv_classh_state;
	} else {
		priv->rcv_hp_classh_state = (unsigned int)priv->rcv_hp_classh_state & (~rcv_classh_state);
	}
	return 0;
...
static void hi6403_headphone_on(struct snd_soc_codec *codec)
{
	...
	priv->rcv_hp_classh_state = (unsigned int)priv->rcv_hp_classh_state | hp_power_state;
...
unsigned int hi6403_calc_res(struct snd_soc_codec *codec, unsigned int voltage_value, unsigned int num)
{
	struct hi6403_platform_data *priv = snd_soc_codec_get_drvdata(codec);
...
static void hi6403_init_chip(struct snd_soc_codec *codec)
{
	...
	if (priv->board_config.classh_rcv_hp_switch)
		priv->rcv_hp_classh_state = (unsigned int)priv->rcv_hp_classh_state | rcv_classh_state;/*lint !e64*/
	else
		priv->rcv_hp_classh_state = (unsigned int)priv->rcv_hp_classh_state & (~rcv_classh_state);/*lint !e64*/
	set_classh_config(codec, priv->rcv_hp_classh_state);
	...
	priv->rcv_hp_classh_state = (unsigned int)priv->rcv_hp_classh_state | hp_classh_state;/*lint !e64*/
		...
		unsigned int virtual_addr)
{
	...
	default :
		ret = 0;
		pr_err("%s : virtual reg addr err, addr=0x%pK\n", __FUNCTION__, (void *)(uintptr_t)virtual_addr);
		break;
	}
