...
static void hplr_pga_power_off(struct snd_soc_component *codec)
{
	...
	snd_soc_component_write(codec, CODEC_ANA_RW5_REG, 0x40);
	snd_soc_component_write(codec, CODEC_ANA_RW5_REG, 0x4C);
	msleep(30);
	snd_soc_component_write(codec, CODEC_ANA_RW5_REG, 0x7C);
	snd_soc_component_write(codec, CODEC_ANA_RW19_REG, 0x8C);
	snd_soc_component_write(codec, CODEC_ANA_RW5_REG, 0xFC);
	snd_soc_component_write(codec, CODEC_ANA_RW24_REG, 0x36);
	snd_soc_component_write(codec, CODEC_ANA_RW20_REG, 0x00);
...
static void ana_dac_power_off(struct snd_soc_component *codec)
{
	...
	/* close  daclr-d */
	snd_soc_component_update_bits(codec, CLK_EN_CFG_REG, ANA_DACLR_EN_MASK, 0x0);
...
static void set_power_codec(struct snd_soc_component *codec, int on)
{
	...
	if (on) {
		...
		snd_soc_component_update_bits(codec, CODEC_ANA_RW22_REG,
			BIT(STB_CTRL_SEC_OFFSET) | BIT(STB_CTRL_OFFSET),
			BIT(STB_CTRL_SEC_OFFSET) | BIT(STB_CTRL_OFFSET));
		snd_soc_component_update_bits(codec, CODEC_ANA_RW23_REG,
...
static void asp_pmu_reg_init(void)
{
	...
	snd_soc_component_update_bits(soc_codec, CTRL_REGB_CLASSD_CTRL1_REG,
...
static void asp_soc_reg_init(void)
{
	...
	snd_soc_component_update_bits(soc_codec, FS_CTRL3_REG,
		I2S1_TDM_FS_MASK, I2S1_TDM_FS_MASK);

	/* set voice dl fs */
	snd_soc_component_update_bits(soc_codec, FS_CTRL0_REG,
		FS_VOICE_L_DLINK_MASK, FS_VOICE_L_DLINK_VALUE);

	/* set voice dr fs */
	snd_soc_component_update_bits(soc_codec, FS_CTRL2_REG,
		FS_VOICE_R_DLINK_MASK, FS_VOICE_R_DLINK_VALUE);

	/* set bt l rx pga gain */
	snd_soc_component_update_bits(soc_codec, BT_L_RX_PGA_CTRL_REG,
		BT_L_RX_PGA_GAIN_MASK, BT_L_RX_PGA_GAIN_VALUE);

	/* set bt r rx pga gain */
	snd_soc_component_update_bits(soc_codec, BT_R_RX_PGA_CTRL_REG,
		BT_R_RX_PGA_GAIN_MASK, BT_R_RX_PGA_GAIN_VALUE);

	/* set i2s1 tdm tx sel */
	snd_soc_component_update_bits(soc_codec, I2S1_TDM_CTRL0_REG,
...
void audio_codec_mute_pga(bool mute)
{
	...
	if (mute) {
		...
		if (val != INVALID_REG_VALUE) {
			...
			snd_soc_component_update_bits(soc_codec, DACL_MIXER4_CTRL0_REG, MUTE_DAC_VALUE_MASK, MUTE_DAC_VALUE);
		} else {
		...
		if (val != INVALID_REG_VALUE) {
			...
			snd_soc_component_update_bits(soc_codec, DACR_MIXER4_CTRL0_REG, MUTE_DAC_VALUE_MASK, MUTE_DAC_VALUE);
		} else {
			AUDIO_LOGE("read dacl reg fail");
		}
		...
		if (customize->mute_dacl_reg_val != INVALID_REG_VALUE) {
			snd_soc_component_update_bits(soc_codec, DACL_MIXER4_CTRL0_REG, MUTE_DAC_VALUE_MASK, customize->mute_dacl_reg_val);
			customize->mute_dacl_reg_val = INVALID_REG_VALUE;
		}

		if (customize->mute_dacr_reg_val != INVALID_REG_VALUE) {
			snd_soc_component_update_bits(soc_codec, DACR_MIXER4_CTRL0_REG, MUTE_DAC_VALUE_MASK, customize->mute_dacr_reg_val);
...
static void da_separate_v5_shutdown(struct platform_device *dev)
{
	...
	/* reset codec */
	snd_soc_component_write(soc_codec, CTRL_REG_PMU_SOFT_RST_REG, 0x22);
...
static void cfg_sif_driving_force(struct snd_soc_component *codec)
{
	...
	snd_soc_component_update_bits(codec, CTRL_REG_DIG_IO_DS_CODEC_CFG_REG,
