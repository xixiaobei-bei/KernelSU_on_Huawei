			    ...
			    const struct task_security_struct *new_tsec)
{
	...
	{
		/*
		 * KernelSU-Next uses u:r:ksu:s0, while some older KernelSU
		 * integrations used u:r:su:s0.  The original bypass only allowed
		 * init -> su, so init executing /data/adb/ksud as ksu still hit
		 * security_bounded_transition() and failed with -EACCES on nosuid.
		 */
		static u32 su_sid;
		static u32 ksu_sid;
		char *secdata;
		...
		if (!error) {
			int rc2 = strcmp("u:r:init:s0", secdata);
			security_release_secctx(secdata, seclen);
			if (rc2 == 0 && (new_tsec->sid == su_sid || new_tsec->sid == ksu_sid))
				return 0;
		}
	}
#endif
